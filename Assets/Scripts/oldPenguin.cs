...
public class oldPenguin : MonoBehaviour
{
    // list of all gameobject needed for AI behaviour of our games.
    ...
    // Enumerator used to determine team.
    ...
    void OnCollisionEnter(Collision other) {
        // =================================================================
        // BLUE TEAM COLLISION BEHAVIOUR BELOW.
        // =================================================================
        if(team == Team.BLUE) {
            ...
            if(other.gameObject.tag == "Red" && enemy_area) {
                ...
                target = GameObject.FindWithTag("RedBase");
                current_action = Actions.FLEE;
                if(other.gameObject.GetComponent<AIBehaviour>().currentAction() == (int) Actions.RESCUE) {
                    other.gameObject.GetComponent<AIBehaviour>().setActions(2);
        ...
        // =================================================================
        // RED TEAM COLLISION BEHAVIOUR BELOW.
        // =================================================================
        else if(team == Team.RED) {
            if(has_flag && other.gameObject.tag == "Red") {
                return;
            }
            if(other.gameObject.tag == "Blue" && enemy_area) {
                if(has_flag) {
                    has_flag = false;
                }
                current_action = Actions.TAGGED;
                game_controller.red_attacker = false;
                GetComponent<Rigidbody>().velocity = Vector3.zero;
            } else if(other.gameObject.tag == "Blue" && has_flag) {
                has_flag = false;
                current_action = Actions.TAGGED;
                game_controller.red_attacker = false;
                GetComponent<Rigidbody>().velocity = Vector3.zero;
